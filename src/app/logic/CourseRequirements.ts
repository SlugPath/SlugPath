...
export function getRequirementsLength(requirements: Requirements): number {
  if (typeof requirements === "string") {
    ...
    return requirements.requirements.reduce((acc, curr) => acc + getRequirementsLength(curr), 0);
  }
}

// CURRENTLY: will only return the courses, and nothing about the requirements groups
export function getCoursesFromRequirements(requirements: Requirements): string[] {
  if (typeof requirements === "string") {
    return [requirements];
  } else {
...
export function courseHasRequirement(course: string, requirement: string, prerequisites: any): boolean {
...
export function removeCoursesWhoseSiblingsHaveItAsRequirement(courses: string[], prerequisites: any): string[] {
